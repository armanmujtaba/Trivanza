...
def generate_itinerary(trip_data):
    ...
    currency = trip_data.get("currency_type", "INR")
    currency_symbol = {
        "₹ INR": "₹",
        "$ USD": "$",
        "€ EUR": "€",
...
2. BUDGET CALCULATIONS:
   - Use total budget: {currency_symbol}{budget_amount} ({budget_type})
   - Calculate EXACT amounts from budget:
     - ✈️ Flights: {flight_pct}% of total = {currency_symbol}{flights}
     - 🏨 Hotels: {hotel_pct}% of total = {currency_symbol}{hotels}
     - 🍽️ Food: {food_pct}% of total = {currency_symbol}{food}
     - 🎡 Activities: 15% of total = {currency_symbol}{activities}
     - 🚖 Local Transport: 10% of total = {currency_symbol}{transport}
     - 🧳 Emergency Fund: 5% of total = {currency_symbol}{emergency}
...
- Budget: {currency_symbol}{budget_amount} ({budget_type})
...
- Time zone adjustments (e.g., Delhi to Europe: {time_zone_diff})
...
**Currency:** {currency_symbol}{budget_amount} ({trip_data.get('currency_type', 'INR')})

## ✈️ FLIGHT DETAILS
**Outbound Journey**
- **6:00 PM IST:** Depart for Indira Gandhi International Airport, Delhi
- **9:00 PM IST:** Flight to {trip_data['destination']} ({currency_symbol}{flights//2} per person)
- **Flight Duration:** Delhi-{trip_data['destination']}: 8h45m
- **Arrival:** Next day {calculate_arrival_time(trip_data)}
- **Airport Transfer:** 1.5-hour taxi to hotel ({currency_symbol}3,000)

**Return Journey**
- **Day {duration}:**
  - **2:00 PM {trip_data['destination']} Time:** Depart for airport
  - **3:00 PM {trip_data['destination']} Time:** Arrive at airport
  - **6:00 PM {trip_data['destination']} Time:** Flight to Delhi ({currency_symbol}{flights//2} per person)
...
- [{trip_data['stay']} in {trip_data['destination']}]({get_search_url('booking', trip_data['destination'], trip_data['stay'])})
  - Price: {currency_symbol}{hotels//duration}/night (total: {currency_symbol}{hotels})
...
**Flight Logistics**
- 6:00 PM IST: Depart Delhi Airport
- 9:00 PM IST: Flight to {trip_data['destination']}
- Arrival: Next day {calculate_arrival_time(trip_data)}
- Airport Transfer: 1.5-hour taxi to hotel

**Afternoon (Post-transfer):**
- [Hotel Name]({get_search_url('booking', trip_data['destination'], 'hotel')}) check-in and rest
- [Local Cuisine Dinner]({get_search_url('zomato', trip_data['destination'], 'local cuisine')}) – {currency_symbol}{food//duration} per person
...
- [Major Attraction]({get_search_url('klook', trip_data['destination'], 'major attraction')}) – {currency_symbol}{activities//duration}
- [Local Activity]({get_search_url('getyourguide', trip_data['destination'], 'local activity')}) – {currency_symbol}{activities//duration}

**Afternoon (12:00 PM - 6:00 PM):**
- [Iconic Restaurant]({get_search_url('zomato', trip_data['destination'], 'iconic restaurant')}) – {currency_symbol}{(food//duration)//1.5}
- [Cultural Experience]({get_search_url('getyourguide', trip_data['destination'], 'cultural experience')}) – {currency_symbol}{(activities//duration)//1.5}

**Evening (6:00 PM - 10:00 PM):**
- [Night Activity]({get_search_url('klook', trip_data['destination'], 'night activity')}) – {currency_symbol}{activities//duration}
- [Local Bar/Nightspot]({get_search_url('zomato', trip_data['destination'], 'bar nightspot')}) – {currency_symbol}{food//duration}

## Day 3 - {all_dates[2].strftime('%A, %B %d')}
**Morning (8:00 AM - 12:00 PM):**
- [Historical Site]({get_search_url('klook', trip_data['destination'], 'historical site')}) – {currency_symbol}{activities//duration}
- [Local Tour]({get_search_url('getyourguide', trip_data['destination'], 'local tour')}) – {currency_symbol}{activities//duration}

**Afternoon (12:00 PM - 6:00 PM):**
- [Local Restaurant]({get_search_url('zomato', trip_data['destination'], 'local restaurant')}) – {currency_symbol}{food//duration}
- [Cultural Activity]({get_search_url('getyourguide', trip_data['destination'], 'cultural activity')}) – {currency_symbol}{activities//duration}

**Evening (6:00 PM - 10:00 PM):**
- [Evening Show]({get_search_url('getyourguide', trip_data['destination'], 'evening show')}) – {currency_symbol}{activities//duration}
- [Casual Dinner]({get_search_url('zomato', trip_data['destination'], 'casual dinner')}) – {currency_symbol}{food//duration}
...
- **6:00 PM:** Flight departure to Delhi ({currency_symbol}{flights//2} per person)
...
- **Airport transfer home:** {currency_symbol}3,000

## 💵 BUDGET BREAKDOWN
- ✈️ Flights: {currency_symbol}{flights} ({flight_pct}% of {currency_symbol}{budget_amount})
- 🏨 Hotels: {currency_symbol}{hotels} ({hotel_pct}% of {currency_symbol}{budget_amount})
- 🍽️ Food: {currency_symbol}{food} ({food_pct}% of {currency_symbol}{budget_amount})
- 🎡 Activities: {currency_symbol}{activities} (15% of {currency_symbol}{budget_amount})
- 🚖 Local Transport: {currency_symbol}{transport} (10% of {currency_symbol}{budget_amount})
- 🧳 Emergency Fund: {currency_symbol}{emergency} (5% of {currency_symbol}{budget_amount})
- 💰 **Total:** {currency_symbol}{budget_amount}
...
Would you like to refine any aspect of this itinerary?"""
    ...
    try:
        response = client.chat.completions.create(
            ...
            temperature=0.3,
            max_tokens=3500   # Increased for full itinerary generation
        )
...
Budget: {st.session_state.trip_context.get("budget_amount", "")} {st.session_state.trip_context.get("currency_type", "INR")}
